...
// Marca o que já existe no vivr pra não importar em dobro quem já foi lançado antes —
// olha exato (data+valor+descrição) e também parecido (parcela, typo, mesma compra
// lançada 2x com poucos dias/centavos de diferença), sem esconder sozinho o que só
// "parece" repetido: isso fica marcado pra pessoa decidir.
function marcarDuplicados(rows){
  ...
  rows.forEach(function(l){
    ...
    if (r.nivel === 'exata') {
      ...
      l.possivelDup = true;
      l.dupInfo = Utils.formatDate(r.match.data) + ' · ' + r.match.descricao + ' · ' + Utils.formatCurrency(r.match.valor);
    }
  });
  return rows;
}
...
function renderRevisao(){
  ...
  var pagamentosTodos = StorageService.getTiposPagamento();
  var responsavelPadrao = document.getElementById('responsavelPadrao').value;
  var pagamentoPadrao = document.getElementById('pagamentoPadrao').value;
  ...
  corpo.innerHTML = linhasDetectadas.map(function(l, i){
    ...
    var wantPagTipo = l.tipo === 'despesa' ? 'pagamento' : 'recebimento';
    var respVal = l.responsavelId != null ? l.responsavelId : responsavelPadrao;
    var pagVal = l.pagamentoId || pagamentoPadrao;
    var respOpts = '<option value="">—</option>' + responsaveis.map(function(r){
      return '<option value="'+r.id+'"'+(r.id===respVal?' selected':'')+'>'+r.nome+'</option>';
    }).join('');
    var pagOpts = pagamentosTodos.filter(function(p){ return p.tipo === wantPagTipo; }).map(function(p){
      return '<option value="'+p.id+'"'+(p.id===pagVal?' selected':'')+'>'+p.nome+'</option>';
    }).join('');
    var badge = '';
    if (l.jaExiste) badge = ' <span class="imp-badge-dup" title="Mesma data, valor e descrição de um lançamento já existente">já lançado</span>';
    else if (l.possivelDup) badge = ' <span class="imp-badge-warn" title="Parecido com um já lançado: '+(l.dupInfo||'').replace(/"/g,'&quot;')+' — confira se não é repetido ou parcela diferente">possível duplicata</span>';
    return '<tr'+(l.jaExiste?' class="imp-dup"':'')+'>'+
      ...
      '<td><input type="text" value="'+l.descricao.replace(/"/g,'&quot;')+'" onchange="setLinha('+i+',\'descricao\',this.value)"/>'+badge+'</td>'+
      '<td><select onchange="setLinha('+i+',\'categoriaId\',this.value)">'+opts+'</select></td>'+
      '<td><select onchange="setLinha('+i+',\'responsavelId\',this.value)">'+respOpts+'</select></td>'+
      '<td><select onchange="setLinha('+i+',\'pagamentoId\',this.value)">'+pagOpts+'</select></td>'+
  ...
  var possiveis = linhasDetectadas.filter(function(l){ return l.possivelDup; });
  ...
  document.getElementById('impSummary').innerHTML =
    '<div>Linhas encontradas<strong>'+linhasDetectadas.length+'</strong></div>'+
...
function importarSelecionados(){
  var pagamentoIdPadrao = document.getElementById('pagamentoPadrao').value;
  var responsavelIdPadrao = document.getElementById('responsavelPadrao').value || null;
  var selecionadas = linhasDetectadas.filter(function(l){ return l.selecionado; });
  ...
  var promessas = selecionadas.map(function(l){
    return StorageService.addLancamento({
      ...
      responsavelId: l.responsavelId || responsavelIdPadrao,
      pagamentoId: l.pagamentoId || pagamentoIdPadrao,
...
/* ── Sincronização com a planilha (Excel/Google Sheets) ──
   Fonte da verdade agora é a planilha que vocês preenchem à mão — aqui a gente só lê o
   CSV publicado, casa com os cadastros existentes (categoria/responsável/pagamento por
   nome) e joga tudo no mesmo fluxo de revisão + detecção de duplicata de sempre. */
...
function buscarLinhasPlanilha(){
  return fetch(PLANILHA_CSV_URL).then(function(r){
    ...
    var idx = {
      ...
      resp: header.indexOf('Responsavel'),
      cat: header.indexOf('Categoria'),
      pag: header.indexOf('Tipo'),
      desc: header.indexOf('Descrição'),
      valor: header.indexOf('Valor (R$)'),
      obs: header.indexOf('Observação')
    };
    if (idx.data === -1 || idx.valor === -1) throw new Error('não reconheci as colunas da planilha (esperava "Data" e "Valor (R$)")');
...
function sincronizarPlanilha(){
  ...
  buscarLinhasPlanilha().then(function(rows){
    if (!rows.length) { setStatus('Não encontrei nenhuma linha válida na planilha.', true); return; }
    setStatus('✓ ' + rows.length + ' linhas na planilha — comparando com o que já foi lançado...');
    linhasDetectadas = marcarDuplicados(rows);
    renderRevisao();
    document.getElementById('stepColar').style.display = 'none';
    document.getElementById('stepRevisao').style.display = '';
  }).catch(function(e){
    setStatus('Não consegui sincronizar a planilha: ' + e.message, true);
  });
}
...
StorageService.initFinanceiro().then(function(){
  ...
  document.getElementById('btnSyncPlanilha').addEventListener('click', sincronizarPlanilha);
